...
(() => {
  ...
  const state = { user: null, route: 'overview', sites: [], groups: [], revisions: [], rollouts: [], events: [], dashboard: null, integrations: [], tokens: [], users: [] };
  ...
  async function renderSettings(){
    ...
    const [integrations,tokens,users]=await Promise.all([
      api('/api/v1/integrations'),
      admin?api('/api/v1/enrollment-tokens'):Promise.resolve([]),
      admin?api('/api/v1/users'):Promise.resolve([])
    ]);
    state.integrations=integrations;state.tokens=tokens;state.users=users;
    const content=document.getElementById('content');
  ...
  function rolloutPayload(form){const fd=new FormData(form),groupId=fd.get('groupId'),startAt=fd.get('startAt'),endAt=fd.get('endAt');return {name:fd.get('name'),revisionId:fd.get('revisionId'),strategy:'waves',waveSize:Number(fd.get('waveSize')),pauseSeconds:Number(fd.get('pauseSeconds')),maxFailures:Number(fd.get('maxFailures')),siteIds:groupId?[]:fd.getAll('sites'),groupId,autoRollback:fd.get('autoRollback')==='on',approvalRequired:fd.get('approvalRequired')==='on',startAt:startAt?new Date(startAt).toISOString():null,endAt:endAt?new Date(endAt).toISOString():null}}
  ...
  document.addEventListener('click', async e => {
    ...
    if(action==='close-modal')closeModal();if(action==='close-drawer')closeDrawer();if(action==='new-revision')openRevisionModal();if(action==='new-rollout')await openRolloutModal();if(action==='new-group')openGroupModal();if(action==='enroll')openEnrollModal();if(action==='add-user')openUserModal();
    if(action==='preview-rollout'){const form=document.getElementById('form-rollout');if(!form)return;try{const plan=await api('/api/v1/rollouts/plan',{method:'POST',body:rolloutPayload(form)});const el=document.getElementById('rollout-plan-preview');if(el)el.innerHTML=`<strong>${plan.total} sites · ${plan.waves} waves</strong><span>${plan.offline||0} currently offline · wave size ${plan.waveSize}</span>`}catch(err){toast(err.message,'error')}return}
  ...
  document.addEventListener('submit', async e => {
    ...
    if(e.target.id==='form-site-edit'){e.preventDefault();const fd=new FormData(e.target);try{await api(`/api/v1/sites/${encodeURIComponent(fd.get('id'))}`,{method:'PATCH',body:{name:fd.get('name'),region:fd.get('region'),labels:parseLabelPairs(fd.get('labels'))}});closeModal();toast('Site metadata updated');await navigate('sites')}catch(err){toast(err.message,'error')}return}
